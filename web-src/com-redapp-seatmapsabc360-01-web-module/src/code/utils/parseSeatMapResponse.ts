...
interface AvailabilityItem {
  label: string;       // 👉 строго в формате "41A"
  price: number;
  currency: string;
  color: string;
}
...
function getSeatColor(seatEl: Element): string {
  ...
  if (isOccupied) return 'gray';        // занято
  if (hasOffer) return 'orange';        // платное
  return 'lightblue';                   // свободное
}
...
export function parseSeatMapResponse(xml: Document): {
  ...
  rowElements.forEach((rowEl, rowIndex) => {
    ...
    seatElements.forEach((seatEl, seatIndex) => {
      ...
      // 🧩 layout нужен только для наглядной отладки
      row.seats.push({
        label: seatLabel,
        x: 60 + seatIndex * 60,
        y: 50 + rowIndex * 40
